...
use test;
 ...
 CREATE TABLE `ENCOUNTERS` (
       `encounterID` int(9) NOT NULL AUTO_INCREMENT,
   	   `cardID` varchar(10) ,
...
CREATE TABLE `USERS` (
	...
	CREATE TABLE `patient_encounters` (
      ...
      `encounter_id` int(6) NOT NULL,
      KEY `patient` (`patient_id`),
      KEY `encounter` (`encounter_id`),
      CONSTRAINT `patients` FOREIGN KEY (`patient_id`) REFERENCES `patients` (`patientID`) ON DELETE CASCADE ON UPDATE CASCADE,
      CONSTRAINT `encounter` FOREIGN KEY (`encounter_id`) REFERENCES `encounter` (`encoutnerID`) ON DELETE CASCADE ON UPDATE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8;
